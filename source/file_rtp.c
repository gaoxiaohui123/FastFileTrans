...
int get_important_packet(FileRtpObj *obj, char *outBuf, short *rtpSize, uint16_t start_seqnum, uint16_t *idx)
{
    //int ret = 0;
...
int file_packet(FileRtpObj *obj, char *out_buf, int out_size)
{
    int ret = 0;
    ...
    unsigned int block_id = obj->block_id;
    ...
    while(offset < data_size)
    {
        ...
        if(flag)
        {
            ...
            rtp_ext->block_id = obj->block_id;
            rtp_ext->rtp_xorcode = 0;
            //
            FileInfo *info_data = (FileInfo *)&out_buf[offset2 + rtp_header_size + ext_size];
            memcpy((void *)info_data, (void *)info, sizeof(FileInfo));
            //
            offset2 += rtp_header_size + ext_size + sizeof(FileInfo);
        }
        else{
...
int file_unpacketFileRtpObj *obj, char *out_buf, int out_size)
{
    int ret = 0;
    FileInfo *info = &obj->info;;
    char *data = obj->data;
		...
		*last_seq_num = seqnum;
		...
		if(rtp_hdr->extension)
		{
            ...
            //get extend info
            {
		        ...
		        //int
		        ref_idc = rtp_ext->ref_idc;
		        int res_num = rtp_ext->res_num;
		        res_idx = rtp_ext->res_idx;
		...
		switch (itype)
		{
			case 1://非IDR图像的片
				...
				offset += rtpSize[idx++];
				pos += n->len;
				//printf("rtp_unpacket: itype=%d, ref_idc=%d \n", itype, ref_idc);
				break;
			case 5://IDR图像的片
				starthead =
				n->startcodeprefix_len = VIDIORASHEADSIZE;
			...
			case 7:	//序列参数集
				...
				{
				    memcpy(&svc_nalu->spsdata[res_idx][0], n->buf, n->len);
				    svc_nalu->sps_size[res_idx] = n->len;
				}
				break;
			case 8://图像参数集
				starthead =
				...
				if(res_idx >= 0 && res_idx < 3)
				{
				    memcpy(&svc_nalu->ppsdata[res_idx][0], n->buf, n->len);
				    svc_nalu->pps_size[res_idx] = n->len;
				}
				//printf("dec IDR = %d \n", IDRCnt);
				//char format1[128] = "dec IDR = %d \n";
				//LogOut(format1, (void *)&IDRCnt);
...
#if 1
	if(!ref_idc && sps_pps_flag != 3)
	{
	    //sps/pps补偿算法
	    if(res_idx >= 0 && res_idx < 3)
	    {
	        //cpy pps
	        int size = svc_nalu->pps_size[res_idx];
	        if(size > 0 && size < 128)
            {
                NALU_t *n = &svc_nalu->nal[0];
                memmove(&n->buf[size], n->buf, n->len);
                memcpy(n->buf, svc_nalu->ppsdata[res_idx], size);
                n->len += size;
                pos += size;
            }
            //cpy sps
	        size = svc_nalu->sps_size[res_idx];
	        if(size > 0 && size < 128)
	        {
	            NALU_t *n = &svc_nalu->nal[0];
	            memmove(&n->buf[size], n->buf, n->len);
	            memcpy(n->buf, svc_nalu->spsdata[res_idx], size);
	            n->len += size;
	            pos += size;
	        }
